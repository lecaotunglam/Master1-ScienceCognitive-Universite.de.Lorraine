...
class MonkeyModelTestCase(unittest.TestCase):
    ...
    def test_constructor(self):
        ...
        real = Monkey()
        test = MonkeyModelTestCase()
        assert isinstance(real, test)


    def test_str(self):
        """testing the representation matches expections"""
        real = Monkey().__str__()
        test = MonkeyModelTestCase().expected_str
        assert isinstance(real, test)

    def test_check_fur(self):
        """testing checkhexacolor is correctly applied"""
        real = Monkey().fur_color
        test = MonkeyModelTestCase().expected_furcolor
        assert isinstance(real, test)

    def test_bmi(self):
        """testing Body Mass Index computations"""
        real = Monkey().compute_bmi()
        test = MonkeyModelTestCase().compute_bmi()
        assert isinstance(real, test)
